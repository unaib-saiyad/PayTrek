      ...
      <div className="px-2 h-[50vh] overflow-auto">
        <table className="w-full border-collapse">
          ...
          <tbody className="dark:bg-gray-800 dark:text-gray-300">
            {data.map((item) => (
              <tr
                key={item._id || item.autoGeneratedKey}
                className={`border-b ${item.isActive === false ? "bg-red-300 dark:bg-red-700" : ""}`}
              >
                {columns.map((col) => (
                  <td key={(item._id || item.autoGeneratedKey) + col.field} className="py-2">
                    {col.type==='time' ?(new Date(item[col.field]).toLocaleDateString()==='Invalid Date'?"-":new Date(item[col.field]).toLocaleDateString()):(item[col.field])} {col.suffix || ""} { col.suffixField? item[col.suffixField]:"" }
                  </td>
                ))}
                <td className="py-2">
                  {onView && (
